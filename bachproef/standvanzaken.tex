...
Voor toepassingen zoals real-time \ce{CO2}-monitoring in schoollokalen, zijn deze nadelen niet wenselijk. Daarom wordt in deze bachelorproef onderzocht of Edge Computing hiervoor een oplossing kan bieden.
...
\subsection{Eigenschappen van de data en keuze van technieken}
...
Deze technieken passen dus goed bij de kenmerken van de gegenereerde data en bij de manier waarop het systeem is opgebouwd in deze proof of concept.
...
Redis is een snelle, in-memory key-value datastore zonder ondersteuning voor SQL, joins of complexe queries. Gegevens worden rechtstreeks benaderd via sleutels, wat Redis zeer geschikt maakt voor real-time toepassingen met hoge snelheidsvereisten. Omdat Redis gegevens standaard in het geheugen bewaart, is het minder geschikt voor permanente opslag: data kan verloren gaan bij een herstart als persistente opslag niet correct is geconfigureerd \autocite{RedisDocumentation}.

\paragraph{Toepassing in sensornetwerken}  
Redis wordt in deze context vooral gebruikt als tijdelijke buffer of cache dicht bij de edge. Dankzij de lage latentie en hoge doorvoersnelheid kunnen gegevens van sensoren snel worden verwerkt. Dit is ideaal voor toepassingen zoals real-time waarschuwingen (bv. plots hoge \ce{CO2}-waarden), tijdelijke opslag van recente metingen, of lokale filtering. Voor structurele opslag over langere periodes wordt Redis vaak gecombineerd met databases zoals PostgreSQL of TimescaleDB, die beter geschikt zijn voor tijdreeksen en permanente opslag \autocite{Mahmud2020, RedisDocumentation}.
...
\begin{table}[H]
    ...
    \textbf{Gegevenstypen} & Key-Value, String, List, Set, Hash, Stream \\ \hline
    \textbf{Geheugen en resourcegebruik} & Zeer laag; volledig in-memory \\ \hline
    \textbf{Distributiemodel} & Redis Cluster voor sharding en replicatie \\ \hline
    \textbf{Schaalbaarheid en prestaties} & Extreem snel, schaalbaar via sharding \\ \hline
    \textbf{Beheer en onderhoud} & Eenvoudige configuratie, monitoring via Redis CLI en GUI-tools \\ \hline
    \textbf{Beschikbaarheid en betrouwbaarheid} & Ondersteunt replicatie; data verdwijnt bij reboot zonder persistente opslag \\ \hline
    \textbf{Ondersteuning en documentatie} & Actieve community, uitgebreide officiële documentatie \\ \hline
    \end{tabular}%
    }
\end{table}
...
\section{Evaluatie van databases binnen Edge Database-architecturen}
    ...
    De onderstaande tabel geeft een overzicht van de scores van vijf databases op de eerder gedefinieerde niet-functionele eisen volgens het MoSCoW-principe.  
    Elke eigenschap werd gescoord van 1 (minst geschikt) tot 5 (meest geschikt). De totaalscore geeft een indicatie van de algemene geschiktheid van een database in edge-omgevingen, met een maximum van 55 punten.  
    ...
    \begin{table}[H]
        ...
        \textbf{Criteria} & \textbf{PostgreSQL} & \textbf{Cassandra} & \textbf{Redis} & \textbf{TimescaleDB} & \textbf{MongoDB} \\ \hline
            Performantie (lage latentie) & 3/5 & 5/5 & 5/5 & 4/5 & 4/5 \\ \hline
            ...
            Lokale verwerking & 3/5 & 5/5 & 4/5 & 5/5 & 4/5 \\ \hline
            Geheugengebruik en CPU-belasting & 3/5 & 4/5 & 5/5 & 4/5 & 4/5 \\ \hline
            Doorvoersnelheid & 4/5 & 5/5 & 5/5 & 4/5 & 4/5 \\ \hline
            Gegevensconsistentie & 5/5 & 4/5 & 3/5 & 5/5 & 4/5 \\ \hline
            Beheer en onderhoud & 4/5 & 3/5 & 4/5 & 4/5 & 3/5 \\ \hline
            Beveiliging & 5/5 & 4/5 & 3/5 & 5/5 & 4/5 \\ \hline
            Automatische compressie en retentie & 3/5 & 3/5 & 2/5 & 5/5 & 4/5 \\ \hline
            \textbf{Totaalscore} & \textbf{40/55} & \textbf{53/55} & \textbf{46/55} & \textbf{52/55} & \textbf{51/55} \\ \hline
        \end{tabular}%
        }
    \end{table}
